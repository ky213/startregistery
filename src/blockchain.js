  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (self.height !== -1) {
          const previousBlock = self.chain[self.chain.length]
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (elapsedTime > 1000 * 60 * 5)
          // 5 minutes
          reject('Eleapsed time is greater than 5 minutes')
        ...
        reject('Error adding the start! ', error.message)
      }
    })
  }
  ...
  getStarsByWalletAddress(address) {
    ...
    return new Promise((resolve, reject) => {
      self.chain.forEach(async block => {
        ...
        if (data.owner === address) stars.push(data.star)
      })
      resolve(stars)
    })
  }
